...
const pkmnMoreThanOneType = pokémon.filter( p => p.types.length > 1);
...
const pkmnPoison = pokémon.filter( p => p.types.length === 1).filter( p => p.types.includes("poison")).map( p => p.name);
console.log(pkmnPoison);

const pkmnFlying = pokémon.filter( p => p.types[1] === "flying").map( p => p.types[0]);
console.log(pkmnFlying);

const pkmnCount = pokémon.filter( p => p.types.includes("normal"));
